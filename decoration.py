#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 20 16:07:45 2020

@author: anjalishaw
"""

import random
black=(0,0,0)
white=(255,255,255)
peach=(255,125,125)
red=(255,0,0)
blue=(0,0,255)

import pygame
x=pygame.init()
pygame.mixer.init()
#pygame.mixer.music.load('name of music.mp3')
#pygame.mixer.music.play()

#window
screen_w=1200
screen_h=800
game_window=pygame.display.set_mode((screen_w,screen_h))
pygame.display.set_caption("real snakes")
bg=pygame.image.load('snake.jpg')
bg=pygame.transform.scale(bg,(screen_w,screen_h)).convert_alpha()
wc=pygame.image.load('welcome.jpg')
wc=pygame.transform.scale(wc,(screen_w,screen_h)).convert_alpha()
pygame.display.update()

clock=pygame.time.Clock()

font=pygame.font.SysFont(None,65)    

def welcome():
    pygame.mixer.init()
    pygame.mixer.music.load('welcome.mp3')
    pygame.mixer.music.play()
    exit_game=False
    while not exit_game:
        game_window.blit(wc,(0,0))
        show_text('welcome to snakes',blue,200,300)
        show_text('press enter to play',blue,200,500)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                exit_game=True
            if event.type ==pygame.KEYDOWN:
                if event.key ==pygame.K_RETURN:   
                    game_loop()
        pygame.display.update()
        clock.tick(100)
    pygame.quit()
    quit()    
def show_text(text,color,x,y):
    text_show=font.render(text,True,color)
    game_window.blit(text_show,(x,y))
    
def plot_snake(game_window,color,snake_list,snake_size):
    for x,y in snake_list:
        pygame.draw.rect(game_window,white,[x,y,snake_size,snake_size])

#gameloop
def game_loop():
    with open ('highscore.txt' ,'r+') as f:
        highscore=f.read()  
    #game specific variables
    exit_game=False
    game_over=False
    score=0
    snake_x=45
    snake_y=45
    
    food_x=random.randint(0,screen_w-20)
    food_y=random.randint(0,screen_h-20)
    
    velocity_x=0
    velocity_y=0
    snake_size=10
    fps=100
    
    snake_list=[]
    snake_length=1

    while not exit_game:
        if game_over:
            pygame.mixer.music.load('gameover.mp3')
            pygame.mixer.music.play()
            game_window.fill(white)
            show_text('GAMEOVER! \n Press enter to continue.',peach,0,350)
            
            with open ('highscore.txt' ,'r+') as f:
                f.write(str(highscore))
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    exit_game=True
            
                if event.type ==pygame.KEYDOWN:
                    if event.key ==pygame.K_RETURN:   
                        welcome()

        else:
#            pygame.mixer.music.load('game.mp3')
#            pygame.mixer.music.play()
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    exit_game=True
                if event.type ==pygame.KEYDOWN:
                    if event.key ==pygame.K_RIGHT:
                        velocity_x+=2
                        velocity_y=0
                    if event.key ==pygame.K_LEFT:
                        velocity_x-=2
                        velocity_y=0
                    if event.key ==pygame.K_UP:
                        velocity_y-=2
                        velocity_x=0
                    if event.key ==pygame.K_DOWN:
                        velocity_y+=2
                        velocity_x=0
                    if event.key ==pygame.K_q:
                        exit_game=True
                    if event.key ==pygame.K_p:
                        score+=10

                        
            if abs(snake_x-food_x)<12 and abs(snake_y-food_y)<12:
                score+=10
                print(score)
                snake_length+=5
                food_x=random.randint(0,screen_w-20)
                food_y=random.randint(0,screen_h-20)
                if score> int(highscore):
                    highscore=score
                
                
            snake_y+=velocity_y
            snake_x+=velocity_x
          #creating a windowfeature for game      
            game_window.fill(white)
            game_window.blit(bg,(0,0))
            head=[]
            head.append(snake_x)
            head.append(snake_y)
            
            snake_list.append(head)
            '''
             snake_y+=velocity_y
             snake_x+=velocity_x
             
             with the  following code list head and so list snake_list keeps incrementing and so snake length 
             as snake_list is directly used to plot the snake
             so we are using
             if len(snake_list)>snake_length:
                del snake_list[0]
            todelete the unnecessary increment in snake's length'''
          
            if len(snake_list)>snake_length:
                del snake_list[0]
                
            if snake_x<0 or snake_x>screen_w or snake_y<0 or snake_y>screen_h:
                game_over=True
                
            if head in snake_list[:-1]:
                game_over=True
                
            
            pygame.draw.rect(game_window,red,[food_x,food_y,snake_size,snake_size])
            
            plot_snake(game_window,white,snake_list,snake_size)
           # pygame.draw.rect(game_window,black,[snake_x,snake_y,snake_size,snake_size])
            show_text("score:" + str(score)+'highscore:' + str(highscore),peach,5,5)
        clock.tick(fps)
        pygame.display.update()
    
    
    pygame.quit()
    quit()
welcome()
